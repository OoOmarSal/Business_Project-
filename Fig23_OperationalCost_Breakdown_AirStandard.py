...
OUTPUT = os.path.join(
    "outputs",
    "Graph23_OperationalCost_Breakdown_AirStandard.png",
)

MODE_SHEET = "Air_Standard"
...
def plot_operational_costs(comparison):
    ...
    labels = [
        AIRPORT_LABELS.get(gateway, gateway)
    ...
    ax.set_title(
        "Operational Cost Breakdown by Airport\n"
    ...
    if np.allclose(security, 0):
        fig.text(
            0.5,
            0.08,
            "Note: Security cost is recorded as £0 for all Air Standard airports in the source data.",
            ha="center",
            fontsize=9,
            color="#555555",
        )

    fig.subplots_adjust(left=0.23, right=0.96, top=0.84, bottom=0.24)
